# MecanumRobot —— Arduino 下位机（麦克纳姆轮机器人）

Arduino 下位机固件，由上位机 ESP32（本板 `bread-compact-wifi-s3cam-airobot`）通过串口控制。

> 该 `.ino` 由 **Arduino IDE / arduino-cli** 编译，烧录到 **Arduino 板**，**不参与** ESP-IDF 固件编译。

## 硬件

- **Arduino 板**：UNO（2KB SRAM）
- **Emakefun 电机驱动板**（I2C 地址 `0x60`）：
  - 4 个直流电机（麦克纳姆轮）：`motors[0~3]`（前左/前右/后左/后右）
  - 2 个舵机：`servo1`（头部）、`servo2`
- **PS2 手柄**：`config_gamepad(13, 11, 10, 12)`，**可选**（未接时 `ps2_ready=false`，手柄轮询被跳过，见下文「注意事项」）
- **蜂鸣器**：A0（`NewTone`）

## 与 ESP32 连接

| Arduino ←→ ESP32 | 说明 |
|---|---|
| **RX ← GPIO43** | ESP32 UART0 TX 发指令 |
| **TX → GPIO44** | ESP32 UART0 RX（**双向回执**，需接线）|
| GND | 共地 |
| 波特率 | **115200** |

> 命令以 **`@`** 开头（如 `@go-forward-3\n`），其余行（如 ESP32 日志）会被 Arduino 忽略。

## 依赖库

需安装到 Arduino 库目录 `~/Documents/Arduino/libraries`：

| 库 | 头文件 | 来源 |
|----|--------|------|
| `MotorDriverBoard`（Emakefun）| `Emakefun_MotorDriver.h` | https://github.com/emakefun/MotorDriverBoard |
| `NewTone` | `NewTone.h` | https://bitbucket.org/teckel12/arduino-new-tone（作者原版）|
| `PS2X_lib` | `PS2X_lib.h` | https://github.com/madsci1016/Arduino-PS2X |

> `PS2X_lib` 源文件在 `Arduino-PS2X/PS2X_lib/` 子目录里，需**把该子目录提升为顶层库目录** `~/Documents/Arduino/libraries/PS2X_lib` 才能被 arduino-cli 索引。

## 编译 & 烧录

### 0. 先确认端口

```bash
arduino-cli board list
```

输出示例：

```
Port  Protocol  Type             Board Name  FQBN  Core
COM3  serial    Serial Port (USB) Unknown
COM1  serial    Serial Port       Unknown
```

- Arduino 板的端口 = 标着 **`Serial Port (USB)`** 的 CH340 端口（示例为 **COM3**）；
- **`COM1` 是主板通信口**，不要选它；
- 换过 USB 口 / 重插后端口号可能变化，每次烧录前先跑一次确认。

### 方式一：一键脚本（推荐，Windows）

sketch 目录下双击或命令行运行：

```bat
build_arduino.bat              :: 仅编译
build_arduino.bat COM5         :: 编译 + 烧录到 COM5
```

> 脚本已内置 `-v`（详细日志）；烧录前用 `arduino-cli board list` 确认实际端口（**别用 COM1**，那是主板通信口）。

### 方式二：arduino-cli

**一条命令（编译 + 烧录）：**

```bash
# Windows 端口用 COMx，Linux/macOS 用 /dev/cu.usbmodemXXXX
arduino-cli compile -v --upload -p COM5 --fqbn arduino:avr:uno \
  main/boards/bread-compact-wifi-s3cam-airobot/arduino/MecanumRobot
```

**分开执行（仅编译 / 仅烧录）：**

```bash
# 仅编译（Arduino UNO，-v 查看详细日志）
arduino-cli compile -v --fqbn arduino:avr:uno main/boards/bread-compact-wifi-s3cam-airobot/arduino/MecanumRobot

# 仅烧录（端口按实际修改，-v 查看烧录详细过程）
arduino-cli upload -v -p COM5 --fqbn arduino:avr:uno main/boards/bread-compact-wifi-s3cam-airobot/arduino/MecanumRobot
```

> 不加 `-v` 时只显示概要结果（编译成功/失败、Flash/RAM 占用）；加 `-v` 可看完整中间过程（库解析/编译命令/烧录握手），排障时建议保留。

### 方式三：Arduino IDE

打开 `MecanumRobot.ino` → 库管理器安装 3 个库 → 选 `Arduino UNO` 板和端口 → 编译烧录。

### RX 缓冲（SERIAL_RX_BUFFER_SIZE）

`arduino:avr 1.8.8+` 无 `setRxBufferSize` API，RX 缓冲由编译期宏 `SERIAL_RX_BUFFER_SIZE` 控制（默认 64B≈4-5 条指令，AI 长指令序列会溢出丢指令）。

- **本机已配置**：核心目录 `platform.local.txt`（内容 `compiler.cpp.extra_flags=-DSERIAL_RX_BUFFER_SIZE=256`），上述命令自动生效，无需额外参数。
- **换电脑 / 重装核心后**：在核心目录 `D:\Arduino15\packages\arduino\hardware\avr\1.8.8\` 重建 `platform.local.txt`，或编译时加：
  ```bash
  arduino-cli compile -v --build-property "compiler.cpp.extra_flags=-DSERIAL_RX_BUFFER_SIZE=256" ...
  ```

## 上位机串口指令（ESP32 下发，均以 `@` 开头）

| 指令 | 效果 |
|------|------|
| `@go-{action}-{steps}` | 动作：forward/back/left/right/leftmove/rightmove/leftup/rightup/leftdown/rightdown（执行完**显式停车**）|
| `@servo-{degree}` | 设置舵机1 角度(0-180) |
| `@servo-home` | 舵机1 回正（回到当前回正角 `servo1Zero`）|
| `@servo-home-set {degree}` | 设置回正角并回正（上位机写入 NVS 后下发）|
| `@speed-{value}` | 设置电机速度(70-255) |
| `@tj-yaotou` / `@tj-shandian` / `@tj-zhuanquan` / `@tj-sxzw` / `@tj-diaotou` | 特技动作（执行完**显式停车**）|
| `@line-start` / `@line-stop` | 巡线模式（沿地面黑线自动行驶）开始/停止 |
| `@drive-{action}-{speed}` | web 摇杆驾驶模式（非阻塞连续控制，需上位机周期心跳保活，1500ms 无心跳自动停）|
| `@drive-stop` | 退出 web 驾驶模式并停止 |

**双向回执（Arduino → ESP32）**：耗时动作（`go-*` / `tj-*` / 巡线）开始执行时回传 `@busy`，执行完毕回传 `@done`；ESP32 的 UART0 RX 解析任务维护状态，供 web 页面 `/uno` 接口读取（不暴露给 AI，避免每次动作后多一轮云端工具调用）。

> 巡线功能参数（基础速度 / 转向强度 / 丢线判定等）与传感器接线详见板级 README「巡线（4 路循迹传感器）」一节。

## 巡线：探头 / 指示灯 / 动作 对照表

用大白话说清楚「哪个灯灭 → 车该往哪拐」，调车和排查时对着看即可。

探头从左到右是 **1、2、3、4 号**（对应代码 `LINE_S1..LINE_S4` = `D7 / D4 / D3 / D2`，见 `MecanumRobot.ino` 顶部宏）。

> **图例**：●＝这盏灯**灭**（这个探头踩到黑线）　○＝这盏灯**亮**（这个探头下面是白地）

### 一个探头 + 一盏灯

| 探头下面是什么 | 灯 | 探头给出的电压 |
|------|------|------|
| **黑线** | **灭** | 4.5V（高）|
| **白地** | **亮** | 0V（低）|

一句话：**灯灭 = 这个探头踩到黑线了；灯亮 = 这个探头下面是白地。**
（与代码 `LINE_ACTIVE = true`（压黑线=高电平）一致。）

### 车跑一趟的模拟（从左到右 1 2 3 4）

| 步骤 | 1 | 2 | 3 | 4 | 谁踩到线 | 车该做的动作 |
|:--:|:--:|:--:|:--:|:--:|---|---|
| **1 起步** | ○ | ● | ● | ○ | 2、3 号 | **直走** |
| 2 车往右飘了 | ○ | ● | ○ | ○ | 2 号 | 往**左**拐一点 |
| 3 飘得更厉害 | ● | ○ | ○ | ○ | 1 号 | **使劲往左拐** |
| 4 拐回来了 | ○ | ● | ● | ○ | 2、3 号 | 回到 **直走** |
| 5 车往左飘了 | ○ | ○ | ● | ○ | 3 号 | 往**右**拐一点 |
| 6 飘得更厉害 | ○ | ○ | ○ | ● | 4 号 | **使劲往右拐** |
| 7 拐回来了 | ○ | ● | ● | ○ | 2、3 号 | 回到 **直走** |
| 8 线跑丢了 | ○ | ○ | ○ | ○ | **一个都没有** | **朝刚才线的方向原地转着找线**（0.8 秒还找不到才停）|
| 9 压到一大片黑 | ● | ● | ● | ● | 4 个全踩到 | **直走**（当路口处理）|

### 动作对照表（看到图形就知道干嘛）

| 看到的图形 | 意思 | 动作 |
|------|------|------|
| ○ ● ● ○ | 线在正中间 | **直走** |
| ○ ● ○ ○ | 线在左边一点 | 往左拐一点 |
| ● ○ ○ ○ | 线在最左边 | **使劲往左拐** |
| ○ ○ ● ○ | 线在右边一点 | 往右拐一点 |
| ○ ○ ○ ● | 线在最右边 | **使劲往右拐** |
| ○ ○ ○ ○ | 线不见了 | **朝最后看到线的方向原地转着找线**；0.8 秒还找不到才停 |
| ● ● ● ● | 压在大片黑上（路口）| 直走 |

> **一句话规则**：**哪边灯灭，线就在哪边，车就往哪边拐；一直让「灭的灯」待在中间（2、3 号位置）。**
> 正常跑的时候，灭的灯会在 2、3 号之间来回跳，这就是「压着线走」。

### 排查：把车停在黑线上别动，看那 4 盏灯

| 看到的现象 | 说明 |
|------|------|
| 只有中间 1~2 盏灭 | 正常 |
| **4 盏全亮** | 探头根本没踩到线：装太高 / 线太细或不在探头下面 |
| **4 盏全灭** | 压在整片黑上，或灵敏度（旋钮）没调好 |
| **换哪个探头压线都一样、看不出区别** | 探头信号没到 Arduino 这 4 个引脚：先查接线（`S1→D7`、`S2→D4`、`S3→D3`、`S4→D2`），再查探头离地高度（推荐 **0.8cm** 左右）|

> 代码行为对照：`readLinePosition()` 全灭返回 `99`（丢线 → 朝最后方向原地转着找线）、全亮返回 `100`（路口 → 直走）；1~3 路踩线时按加权位置算偏差做比例差速（`LINE_KP`），偏差到 ±3（只有最外侧压线）时改用原地猛拐。

## 代码实际动作 + 日志怎么看

`readLinePosition()` 把 4 路读数算成一个偏差 `pos`（`-3`~`+3`，负数=线偏左，正数=线偏右；`99`=全白丢线，`100`=全黑路口），`lineFollowOnce()` 每轮按 `pos` 驱动：

| 探头状态 | pos | 左轮 / 右轮（当前参数）| 日志（动作变化时才打一行）|
|------|:--:|------|------|
| ○ ● ● ○（线在中间）| 0 | 90 / 90（直走）| `巡线: 前进 (0) 探头[0110]` |
| ○ ● ○ ○（线偏左一点）| -1 | 50 / 130 | `巡线: 左拐 (-1) 探头[0100]` |
| ○ ○ ● ○（线偏右一点）| +1 | 130 / 50 | `巡线: 右拐 (1) 探头[0010]` |
| 同侧压 2 路（偏得多）| -2 / +2 | 20 / 170、170 / 20 | `巡线: 左拐 (-2)` / `巡线: 右拐 (2)` |
| ● ○ ○ ○ / ○ ○ ○ ●（只有最外侧压线）| -3 / +3 | **原地猛拐**：一侧两轮倒转、另一侧正转 | `巡线: 原地左拐 (-3)` |
| ○ ○ ○ ○（线丢了）| 99 | **朝最后看到线的方向原地转着找线**，超 800ms 才停 | `巡线: 丢线往左找 探头[0000]` |
| ● ● ● ●（一片黑 / 路口）| 100 | 90 / 90（直走）| `巡线: 路口直行 探头[1111]` |

- 日志末尾的 `探头[0110]` 是**原始四路电平**（S1~S4，`1`=压到黑线），弯道 / 直角 / 丢线时直接看这一行，就知道车"看到"了什么。
- **动作没变就不重复打**（并限速 100ms），所以日志不会刷屏；「停止」一定会打。
- 机器人**不再**在巡线时刷 `F`：`moveForward()` 打的 `F` 现在只属于 `go-forward` 等点动命令。
- 注意：普通巡线（1~3 路踩线）**只在动作变化时**打日志，所以串口安静下来就说明车正稳定地压着中间走。

**可调参数**（`MecanumRobot.ino` 文件顶部宏）：

| 宏 | 当前值 | 作用 |
|------|:--:|------|
| `LINE_BASE_SPEED` | 90 | 基础速度（线在正中时两轮都给这个值）|
| `LINE_KP` | 40 | 转向强度：偏 1 路差速 ±40，偏 2 路 ±80 |
| `LINE_MIN_SPEED` | 20 | 内侧轮最低速（防一下停死）|
| `LINE_PIVOT_MAG` | 3 | 偏差达到几才用「原地猛拐」|
| `LINE_PIVOT_SPEED` | 110 | 原地猛拐的转速 |
| `LINE_LOST_MS` | 800 | 丢线后还继续找线多久（超时 → 停车 + `@done line-follow`）|
| `LINE_MAX_MS` | 120000 | 单次巡线总时长上限（2 分钟，防死循环）|

**调参经验**：
- 弯道**拐不过来** → 加大 `LINE_KP`（或减小 `LINE_BASE_SPEED`）
- 弯道**扭来扭去** → 减小 `LINE_KP`（力度跳变太大就会左右摆）
- 起步 / 直线就冲出去 → 减小 `LINE_BASE_SPEED`

> 现场排查顺序：先看日志里的 `探头[...]`（信号有没有进来）→ 再看动作（前进 / 左拐 / 右拐 / 原地左拐 / 丢线往左找）→ 最后才调参数。

## PS2 手柄键位

| 按键 | 功能 |
|------|------|
| 十字键 ↑↓←→ | 前进/后退/左移/右移 |
| PINK / RED | 原地左转 / 原地右转 |
| GREEN / BLUE（单击）| 降速 / 加速 |
| L1 / R1 / L2 / R2 | 左前斜 / 右前斜 / 左后斜 / 右后斜 |
| SELECT / START | 舵机1 微调(-2 / +2) |

## 注意事项

- **看日志 / 烧录 ESP32 时请先断开 Arduino 与 ESP32 的接线**（ESP32 板载 USB-UART 走 GPIO43/44，与 Arduino 共用会干扰）。
- 程序只认 **`@` 开头** 且带 `\n` 结尾的行，其余（如 ESP32 日志）一律忽略。
- 解析用**固定 `char` 缓冲**（不用 `String`），适合 UNO 2KB SRAM，抗内存碎片。
- **点动命令必须显式停车**：`moveForward()` 等只做 `runMotors(方向, t)` + `delay(t)`，**自身不停车**，`handleCommand()` 的 `go-*`/`tj-*` 分支末尾必须 `stopMove(0)`。**不要**依赖 `handleGamepad()` 无手柄时每轮 `stopMove(10)` 的副作用——它只对“无按键”生效，且 `ps2_ready=false` 时已直接返回；一旦被删，AI 动作执行完电机会一直转（板级 README 踩坑 5）。
- **PS2 手柄未接时必须跳过轮询**：`setup()` 里 `ps2_ready = (ps2x.config_gamepad(...) == 0)`，`handleGamepad()` 开头 `if (!ps2_ready) return;`。否则 `read_gamepad()` 每轮失败重试 5 次 + 每次 `reconfig_gamepad()`（`read_delay` 升至 10），**单次阻塞约 250ms**，loop 周期变 ~300ms，web/AI 指令全部延时（板级 README 踩坑 6）。
- **烧录失败排查**：Windows + CH340 的 UNO 克隆板上传报 `cannot set com-state` 是 CH340 新版驱动（3.8+）在 Win11 的已知 bug，降级驱动到 3.5.2019.1 即可——详见板级 README「踩坑记录：CH340 新驱动导致 Arduino 上传失败」。
