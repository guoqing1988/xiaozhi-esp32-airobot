...
| 激光 | 50ms | 读 DO + 二次采样去抖；**只在边沿变化**时 `evt beam`，被挡才播 `beam` |
...
DHT11 与激光因 GPIO4/5 已被 RGB 占用，改用 **GPIO16 / GPIO17**。
...
| | GND | GND | |
| 激光模块 | VCC | 3V3 | 只取数字输出，不用 ADC |
| | DO | GPIO5 | 高电平 = 被遮挡（触发播报 `beam.mp3`） |
| | GND | GND | |
| | AO | **不接** | 模拟输出用不到 |
...
一台 ESP32-S3 同时接 RGB 灯 + 超声波 + DHT11 + 激光，
...
| 激光模块 | VCC / GND | 3V3 / GND | |
| | DO | **GPIO17** | **换了脚**：GPIO5 已被 RGB 的 G 占用 |
| | AO | **不接** | |

> **从节点 2 搬线时注意**：DHT11 与激光的线要从 GPIO4/5 **挪到 GPIO16/17**，
...
#   ⚠️ 节点 3 的 DHT11/激光接 GPIO16/17（不是节点 2 的 4/5），接线见上节
...
| 宏 | 默认 | 作用 |
|---|---|---|
| `NODE_ID` | 1 | 1=客厅灯(RGB+超声波)，2=玄关感应(DHT11+激光)，3=融合节点(四件套) |
