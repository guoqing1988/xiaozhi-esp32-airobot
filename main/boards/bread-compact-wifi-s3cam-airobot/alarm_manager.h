...
struct AlarmItem {
    ...
    std::string song;         // 指定铃声歌曲名(空=随机选歌); 落盘
    bool        enabled = true;
...
public:
    ...
    // 增加闹钟(写入文件), 返回新 id。song 为指定铃声歌曲名(空=随机)。
    int Add(AlarmType type, int trigger_sec, const std::string& label, const std::string& song = "");
    // 清除所有闹钟(供 web "清空" 用)
    void ClearAll();
    // 按 id 删除
    bool Remove(int id);
