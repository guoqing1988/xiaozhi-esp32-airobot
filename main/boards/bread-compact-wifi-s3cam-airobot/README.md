...
| 功能区 | 说明 | 详见 |
|--------|------|------|
| 🎵 歌曲上传 | 多选上传 .mp3/.lrc/wav/m4a…、实时进度条、同名覆盖开关 | 「TF 卡本地歌曲播放」|
...
> 页面有四个 tab（歌曲管理 / 闹钟提醒 / 机器人控制 / 照片）——「提示音」不占 tab，就在**歌曲管理**页下方。**调试日志没有单独 tab**，而是收在「🎮 机器人控制」面板底部（下位机指令记录 + 折叠的「系统日志」）；**拍照与照片相册都在「📷 照片」Tab**。
...
- 协议与 Arduino 下位机**同构**（`@` 前缀文本行）：
  ...
  | 主控→节点 | `@n1 do light rgb 0 0 255` | 通用动作，主控原样透传、不解释 |
  | 节点→主控 | `@n1 ok light 0 0 255` | 执行回执（进状态缓存） |
...
8. **主控重启** → 节点在下一轮 beacon 后重报能力，清单自愈；
9. 全程 AI 对话不卡顿、无重启；`free sram` 不低于改动前（注册表约 1.5KB）；
10. 拔掉 TF 卡 → 播报静默跳过，控制与查询仍正常。
