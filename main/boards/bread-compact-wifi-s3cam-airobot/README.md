...
- **原理**：4 路数字输出读线位置（加权 -1.5/-0.5/+0.5/+1.5 → 偏差 `-3`~`+3`）→ 按偏差比例差速（`LINE_KP`）：内侧轮减速、外侧轮加速。偏差到 ±3（只有最外侧探头压线）、或一路都没压到（丢线）时，改用**原地拐**（一侧两轮倒转、另一侧正转），丢线时朝**最后看到线的方向**转着找线。
- **AI 指令**：说「开始巡线 / 沿着线走」→ `self.uno.line_follow(1)`；「停止巡线」→ `(0)`。
- **结束条件**：连续丢线超过 `LINE_LOST_MS`（当前 **800ms**，线断/到终点）或总时长超 `LINE_MAX_MS`（当前 2 分钟）→ 自动停车并回传 `@done line-follow`，AI 会汇报「巡线结束」。
- **可调参数**（`arduino/MecanumRobot/MecanumRobot.ino` 顶部宏）：`LINE_BASE_SPEED` 基础速度（90）、`LINE_KP` 转向强度（40）、`LINE_MIN_SPEED` 内侧轮最低速（20）、`LINE_PIVOT_MAG` 触发原地拐的偏差（3）、`LINE_PIVOT_SPEED` 原地转速（110）、`LINE_LOST_MS` 丢线找人时长（800）、`LINE_MAX_MS` 时长上限。
- **调参方向**：弯道拐不过来 → 加大 `LINE_KP` 或减小 `LINE_BASE_SPEED`；弯道扭来扭去 → 减小 `LINE_KP`；起步就冲出去 → 减小 `LINE_BASE_SPEED`。
