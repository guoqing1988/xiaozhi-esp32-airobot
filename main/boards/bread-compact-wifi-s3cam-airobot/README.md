...
## ESP-NOW 居家灯控与传感器（比赛演示）

本板作为**主控**，通过 ESP-NOW 无线控制 1~2 个自制 **ESP32-S3 节点**上的灯与传感器。
...
②③ 不需要说话，是**传感器事件主动触发播报**，现场最有观赏性（⑤ 高温同理：≥28℃ 播一次，≤26℃ 复位）。

### AI 语音指令（服务端通过 MCP 工具自动调用）

| 你对助手说的话 | 触发工具 | 效果 |
|---|---|---|
| 「打开客厅灯 / 关灯」 | `self.home.light(node=1, on=1/0)` | 开/关灯 |
| 「调成蓝色 / 红色」 | `self.home.light(node=1, r=0, g=0, b=255)` | 改颜色 |
| 「暗一点 / 亮一点」 | `self.home.light(node=1, brightness=80)` | 调亮度 |
| 「室内多少度 / 湿度多少」 | `self.home.sensor(node=2)` | 返回温湿度 JSON（含 `stale`） |
| 「居家节点在线吗」 | `self.home.status` | 节点在线状态 |
| （自测用） | `self.home.announce(name=motion)` | 手动播报一次 |
...
- 文件名固定：`motion.mp3`（有人靠近）、`beam.mp3`（门口有人经过）、`hot.mp3`（温度偏高）；
...
- 协议与 Arduino 下位机**同构**（`@` 前缀文本行）：下行 `@n1 light 1 0 0 255 200`、上行 `@n1 evt temp 26 55`。
- **节点零配置**：不写 SSID/密码/主控 MAC/信道。主控每 500ms 广播一条 `@beacon`（30 秒后转 3 秒稳态），
  节点在信道 1..13 间 hop，收到即锁定当前信道；失联 5 秒自动回 hop（**换热点、双方重启都自恢复，无需重烧固件**）。
- **密钥**：`kPmk`/`kLmk` 必须与节点固件一致（`espnow_home.cc` ↔ `EspNowNode.ino`，各 16 字节）。
  不一致的现象是**完全收不到任何包**（不是偶发失败），排查时优先核对这两处。

### 设计取舍（改这块前先读）

1. **只有上行连发 3 次，下行只发 1 次**：主控待机是 `WIFI_PS_MAX_MODEM`、只在 DTIM 醒来，
   节点单包上报会被漏掉 → 节点连发 3 次（间隔 150ms）+ 主控按 `(node_id, evt)` 1 秒去重；
...
4. **迟滞放节点侧、业务阈值放主控侧**：节点只做抗抖动（超声波 30/40cm 迟滞、激光两次采样一致、
   距离变化 ≥3cm 才上报），阈值（高温 28℃）留主控便于语音调整。
5. **DHT11 读失败不清零**：节点重试 3 次后上报 `evt err dht`，主控**保留上次读数**并标 `stale: true`
   （>30 秒未更新），让 AI 说明"数据可能过期"，而不是拿旧值当实时。
   （DHT11 是单总线，时序会被 WiFi 中断干扰；若读数不稳可换 I2C 的 SHT30/DHT20，
   只需改节点 10 行读取函数，协议与主控侧不变。）

### 真机验证要点

1. 节点上电 → 3 秒内 `self.home.status` 显示 `online: true`；
2. 「打开客厅灯」→ 灯亮；「调成蓝色」→ 变蓝；「关灯」→ 灭；
3. 手靠近超声波 <30cm → 灯亮 + 播报（10 秒内不重复）；
4. 「室内多少度」→ 与实物温度计接近（DHT11 ±2℃）；
5. 手挡激光 → 播报「门口有人经过」；
6. 拔节点电源 30 秒再插 → 自动恢复在线；
7. 主控换热点（信道变化）→ 节点自动重锁定，**无需重烧固件**；
8. 全程 AI 对话不卡顿、无重启；`free sram` 不低于改动前；
9. 拔掉 TF 卡 → 播报静默跳过，控灯与查询仍正常。

### 排错

| 现象 | 检查 |
|---|---|
| 控灯无反应 | `self.home.status` 看该节点 `online`；节点是否上电（hop 找信道需约 3 秒） |
| 一直离线 | 两侧密钥是否一致；节点是否被同信道的 2.4G 流量干扰 |
| 播报不响 | 是否正在**对话中**（对话不插嘴）；TF 卡 `/sdcard/announce/` 是否有对应 mp3 |
| 播报刷屏 | 检查冷却（10 秒）与迟滞阈值是否被调得过近 |
| 温度一直 stale | DHT11 接线/供电；看节点是否在重试后上报 `evt err dht` |
