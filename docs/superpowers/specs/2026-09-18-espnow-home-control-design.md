...
后续修正（ESP-NOW 链路打通，2026-09 现场调试落地）：

| # | 原设计 | 修正为 | 原因 |
|---|---|---|---|
| 14 | 节点上行用**加密单播**（`HomePeer` 带 LMK）；主控侧“收到包后按源 MAC 注册加密对端” | 节点上行改**明文广播**（`tx_peer` 指向广播地址、`lmk` 传 `nullptr`）；加密 `peer` 只用于接收主控下行 | **加密死锁**：ESP-NOW 的加密帧要求接收端**事先**注册同样 LMK 的对端，否则驱动解不开直接丢包。而主控是“收到包之后才注册”→ 双方互等 → 主控永远收不到节点（现象：AI 认不到任何设备）。广播上行不需预注册，先打破死锁，之后主控下发加密命令节点照样能解。与官方 `ESP_NOW_Network` 示例的发现流程一致 |
| 15 | 断开主控时 `delete peer` | 先 `peer->detach()`（= 基类 protected `remove()`）再 `delete` | **基类 `~ESP_NOW_Peer()` 是空的**，只 `delete` 不会把它从 ESP-NOW 对端表摘掉。后果有两个：① 主控广播被当成“已注册对端”，不再触发 `onNewPeerCb`（锁定入口）→ 节点“收到 beacon 却永远不 LOCKED”；② 对端数组留下指向已释放对象的**悬空指针**，主控下行命令走到 `onReceive` 时就是访问野内存 → **命令静默丢失**（这是“命令一条都到不了节点”的真正根因） |
| 16 | `HOP_INTERVAL_MS 200`（每信道 200ms）；`LOST_TIMEOUT_MS 5000` | `HOP_INTERVAL_MS 2000`；`LOST_TIMEOUT_MS 12000` | 主控 beacon 周期 3000ms，节点每信道只停 200ms → 命中率约 7%，基本扫不到主控；2000ms 提到 67%，一圈（26 秒）内基本能锁上。失联判定从 5 秒放宽到 12 秒：主控 3 秒一次广播，偶尔丢两三个很正常，5 秒就回 hop 会反复重连 |
