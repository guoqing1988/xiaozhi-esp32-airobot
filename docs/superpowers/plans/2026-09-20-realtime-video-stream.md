...
- **实施状态（2026-09-20）**：S1–S8 已全部落地；编译通过（`idf.py build`）+ 全量 367 项单测通过（新增视频流契约测试 27 项）。
  **真机未验证**——上板后需确认：出画、帧率/分辨率角标、取消后堆内存回落、切走 Tab 自动断流。
- **用途修正（2026-09-20，用户澄清）**：本功能是**网页 FPV 遥控机器人走位**，**延时第一**；
  实测场景中开视频时**不并行走家里的 ESP-NOW 控制**，所以不再为节点控制让出空口：
  帧率上限 12fps → **20fps**，并补上低延时三件套（`TCP_NODELAY` 禁用 Nagle、
  取帧 `CAMERA_GRAB_LATEST`、落后不补帧）。分辨率保持 VGA（R6 不变）。
  想再降延时：调大 `jpeg_quality` 或把 `frame_size` 降到 QVGA。
